...
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/usr/local/etc/xray/netz/shadowsocksws.json")
...
echo -e "==============================="
grep -E "^### " "/usr/local/etc/xray/netz/ssws.json" | cut -d ' ' -f 2-3 | nl -s ') '
...
user=$(grep -E "^### " "/usr/local/etc/xray/netz/ssws.json" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
exp=$(grep -E "^### " "/usr/local/etc/xray/netz/ssws.json" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/ssws.json
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/sswsnon.json
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/sskuota.json
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/ssgrpc.json
service cron restart
clear
echo ""
echo " SHADOWSOCKS Berhasil Diperpanjang !"
echo " =========================="
