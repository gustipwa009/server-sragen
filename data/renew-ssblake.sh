...
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/usr/local/etc/xray/netz/ssblakews.json")
...
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/ssblakews.json
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/ssblakewsnon.json
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/ssblakekuota.json
sed -i "s/### $user $exp/### $user $exp4/g" /usr/local/etc/xray/netz/ssblakegrpc.json
service cron restart
clear
echo ""
echo " SHADOWSOCKS 2022 Berhasil Diperpanjang !"
echo " =========================="
