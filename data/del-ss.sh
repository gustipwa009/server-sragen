...
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^### " "/usr/local/etc/xray/netz/ssws.json")
...
sed -i "/^### $user $exp/,/^},{/d" /usr/local/etc/xray/netz/ssgrpc.json
rm -f /root/akun/shadowsocks2022/$user.txt
